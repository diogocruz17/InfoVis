function createScatterPlotRelative() {
    // set the dimensions and margins of the graph
    // append the svg object to the body of the page
    var svg = d3.select("#scatterplot_relative"),
    ...
    data = d3.csv("./data/merged_data.csv", function(data) {
        ...
        countries.forEach(function(c) {
            ...
            if (year1.length > 0 && year2.length > 0) { // A country must be present in both years
                if(year1[0][attr1] != 0){
                    var attr1Diff = (year2[0][attr1] - year1[0][attr1]) / year1[0][attr1] * 100
                    var attr2Diff = year2[0][attr2] - year1[0][attr2]
                    if(Math.abs(attr1Diff) < 101){
                        results.push([c, attr1Diff, attr2Diff, year1[0], year2[0], year1[0][attr1]]) 
                    }
        ...
        console.log(Math.floor(minAttr1))
        console.log(Math.floor(maxAttr1))
        ...
        svg.append('g')
            ...
            .attr("r", function(d) { return d[5] * 4;})
            ...
            .on("mouseover", function(d) { 
                ...
                tooltip.append("text").html("<br>Relative change in " + attr1 + ": " + Number((d[1]).toFixed(2)) + "%");
                tooltip.append("text").html("<br>Change in " + attr2 + ": " + Number((d[2]).toFixed(2)));    
                tooltip.append("text").html("<br><br>" + y1["Year"]);
        ...
        const yAxisGrid = d3.axisRight(y).tickSize(width).tickFormat('').ticks(5);
